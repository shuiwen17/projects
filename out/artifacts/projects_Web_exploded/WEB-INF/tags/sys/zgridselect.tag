...
<%@ attribute name="valueName" type="java.lang.String" required="false" description=""%>
<%@ attribute name="valueValue" type="java.lang.String" required="false" description=""%>
<%@ attribute name="imageName" type="java.lang.String" required="false" description=""%>
<%@ attribute name="imageValue" type="java.lang.String" required="false" description=""%>
	<input id="${id}Id" name="${name}" code="goodsId"  type="hidden" value="${value}"/>
	<input id="${id}valueName" name="${valueName}"  type="hidden" value="${valueValue}"/>
	<input id="${id}imageName" name="${imageName}"  type="hidden" value="${imageValue}"/>
		...
		top.layer.open({
		    ...
		    yes: function(index, layero){
		    	 ...
		    	 for(var i=0; i<items.length; i++){
                     ...
                     goodsImages.push(item.goodsImage);
				 ...
				 $("#${id}valueName").val(names.join(","));
		    	 $("#${id}imageName").val(goodsImages.join(","));
		    	 
		    	 //拿到那一行所有的td  并且把值写入到td里
                 var goodsTd = goodsinfo.find("td");
             //   $(goodsTd[1]).html(goodsNames.join(","));
             //   $(goodsTd[3]).html(goodsImages.join(","));
